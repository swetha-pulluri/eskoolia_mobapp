...
Future<void> _exportDashboardCsv(
  ...
  try {
    ...
    await saveBytesForDownload(
      bytes: bytes,
      filename:
          'eskoolia-dashboard-${DateTime.now().toIso8601String().split('T').first}.csv',
    );
    if (context.mounted) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Dashboard exported.')));
    }
  } catch (e) {
    if (context.mounted) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Dashboard export failed: $e')));
    }
  }
}
...
class SuperAdminDashboardPage extends ConsumerWidget {
  ...
  Widget _buildActivityIcon(String action, String severity) {
    ...
    final isUpgrade =
        a.contains('upgrade') || a.contains('plan') || a.contains('migrat');
    final isWarn =
        severity == 'warning' ||
        a.contains('threshold') ||
        a.contains('storage');
    final isDanger =
        severity == 'error' ||
        severity == 'critical' ||
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final planMrrTotal = dashboard.planBreakdown.fold<double>(
      0.0,
      (sum, p) => sum + p.mrr,
    );
    final effectiveMrr = dashboard.mrr.current > 0
        ? dashboard.mrr.current
        : planMrrTotal;
    ...
    return SchoolTenancyLayout(
      child: Container(
        ...
        child: SafeArea(
          ...
          // Admin Home's own scroll behavior exactly, so this page always
          // stays scrollable/refreshable instead of the default physics,
          // which refuses to scroll or bounce at all once content happens
          // to be shorter than the viewport.
                        ...
                        Text.rich(
                          TextSpan(
                            ...
                            children: [
                              ...
                              TextSpan(
                                text:
                                    '${dashboard.totalStudents.toStringAsFixed(0)} students',
                                style: const TextStyle(
                                  fontWeight: FontWeight.w600,
                                  color: AppColors.textPrimary,
                                ),
                          ...
                          children: [
                            OutlinedButton.icon(
                              onPressed: dashboardAsync.isLoading
                                  ? null
                                  : () => ref.invalidate(
                                      schoolTenancyDashboardProvider,
                                    ),
                            ...
                            OutlinedButton.icon(
                              onPressed: () =>
                                  _exportDashboardCsv(context, dashboard),
                            ...
                            ElevatedButton.icon(
                              ...
                              onPressed: () =>
                                  context.go('/super-admin/schools?add=1'),
                  ...
                  CompactKpiCardGrid(
                    ...
                    cards: [
                      ...
                      CompactKpiCard(
                        ...
                        trendColor: AppColors.successGreen,
                        footnote:
                            '${dashboard.totalStudents} total · ${dashboard.totalStaff} staff',
                      ),
                      CompactKpiCard(
                        ...
                        trendColor: AppColors.successGreen,
                        footnote:
                            'GST collected this month · ${_formatINR(effectiveMrr * 0.18)}',
                      ),
                      CompactKpiCard(
                        ...
                        trend: dashboard.alertCount > 0
                            ...
                            : '— All clear',
                        trendColor: dashboard.alertCount > 0
                            ? AppColors.dangerRed
                            ...
                            children: [
                                        ...
                                        children: [
                                          Row(
                                            children: [
                                              ...
                                              Text(
                                                board.board,
                                                style: AppTextStyles.boardLabel,
                                              ),
                                            ],
                                          ),
                                          Row(
                              ...
                              if (dashboard.boardBreakdown.isNotEmpty) ...[
                                ...
                                Wrap(
                                  spacing: 6,
                                  runSpacing: 6,
                                  children: dashboard.boardBreakdown.map((
                                    board,
                                  ) {
                                    return Container(
                                      padding: const EdgeInsets.symmetric(
                                      ...
                                      SizedBox(
                                        ...
                                        child: Row(
                                          ...
                                          children: [
                                            ...
                                            Text(
                                              '· ${_formatStudents(state.students)} st.',
                                              style: AppTextStyles
                                                  .boardPercentage
