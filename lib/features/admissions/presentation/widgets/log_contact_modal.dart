...
class _LogContactModalState extends ConsumerState<LogContactModal> {
  ...
  late String _nextFollowUpDate = DateTime.parse(
    widget.today,
  ).add(const Duration(days: 2)).toIso8601String().substring(0, 10);
  bool _saving = false;
  String? _error;
  ...
  Future<void> _submit() async {
    ...
    final outcomeLabel = kLogOutcomes.firstWhere(
      (o) => o['value'] == _outcome,
    )['label']!;
    ...
    final logEntry =
        '[$timestamp] $outcomeLabel${_note.text.isNotEmpty ? ': ${_note.text}' : ''}';
      ...
      await ref
          ...
          .updateInquiry(
            ...
            (c) => c.copyWith(
              ...
              status: newStatus,
              activeStatus: newStatus == 'enrolled' || newStatus == 'declined'
                  ? 2
                  : 1,
            ),
          );
  ...
  Widget _quickChip(String label, int days) {
    return OutlinedButton(
      onPressed: () => setState(
        () => _nextFollowUpDate = DateTime.parse(
          widget.today,
