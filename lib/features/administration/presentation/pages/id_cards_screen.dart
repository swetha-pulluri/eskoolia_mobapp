...
class _IdCardsScreenState extends ConsumerState<IdCardsScreen> {
            ...
            return SafeArea(
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    const Text('Applicable Roles', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600)),
                    const SizedBox(height: 8),
                    CheckboxListTile(
                      contentPadding: EdgeInsets.zero,
                      title: const Text('All roles', style: TextStyle(fontSize: 13.5, fontWeight: FontWeight.w600)),
                      subtitle: const Text('Use this template for every role in the school', style: TextStyle(fontSize: 11.5)),
                      value: applyAll,
                    ...
                    ...roles.map((role) => CheckboxListTile(
                          contentPadding: EdgeInsets.zero,
                          title: Text(role.name, style: const TextStyle(fontSize: 13.5)),
                          value: selected.contains(role.id),
                          onChanged: (v) => setSheetState(() {
                            applyAll = false;
                            if (v == true) {
                              if (!selected.contains(role.id)) selected.add(role.id);
                            } else {
                    ...
                    const SizedBox(height: 8),
                    WebButton(label: 'Done', onPressed: () => Navigator.of(context).pop()),
                  ],
                ),
              ),
            );
  ...
  Future<void> _submit() async {
    ...
    if (ok) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(_editingId == null ? 'ID card saved successfully.' : 'ID card updated successfully.')),
      );
      _resetForm();
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final selectedNames = _applyToAllRoles
        ? const <String>[]
        : _selectedRoleIds.map((id) => roles.firstWhere((r) => r.id == id, orElse: () => const RoleEntity(id: 0, name: '')).name).where((n) => n.isNotEmpty).toList();
              ...
              child: Column(
                ...
                children: [
                  AdminTextField(
                    ...
                    maxLength: 100,
                    helper: 'Enter a meaningful title (3-100 characters, letters, numbers and spaces).',
                    validator: (v) {
                      final value = v?.trim() ?? '';
                  ...
                  AdminDropdownField<String>(
                    ...
                    value: _layout,
                    hint: 'Select layout',
                    helper: 'Choose the card orientation: Horizontal (landscape) or Vertical (portrait).',
                    items: const [
                      DropdownMenuItem(value: 'horizontal', child: Text('Horizontal')),
                      DropdownMenuItem(value: 'vertical', child: Text('Vertical')),
                    ],
                    onChanged: (v) => setState(() => _layout = v ?? 'horizontal'),
                  ),
                  Padding(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('Applicable Roles', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
                        const SizedBox(height: 6),
                        roles.isEmpty
                            ? Container(
                                padding: const EdgeInsets.all(10),
                                decoration: BoxDecoration(color: const Color(0xFFFEF3C7), border: Border.all(color: const Color(0xFFFBBF24)), borderRadius: BorderRadius.circular(8)),
                                child: const Text('Warning: No roles available. Please configure roles in the system settings first.', style: TextStyle(fontSize: 12.5, color: Color(0xFF92400E))),
                              )
                            : InkWell(
                                ...
                                child: InputDecorator(
                                  ...
                                  child: Text(rolesDisplayText, style: const TextStyle(fontSize: 13.5)),
                                ),
                              ),
                        const SizedBox(height: 4),
                        const Text('Pick specific roles or choose All roles to apply this ID card to everyone.', style: TextStyle(fontSize: 11, color: AppColors.textTertiary)),
                      ],
                    ),
                  ),
                  AdminFileField(fileName: _bgFrontName, placeholder: 'Background Image (Front) — PNG, JPG (max 2MB)', onTap: () => setState(() => _bgFrontName = 'front_bg.jpg')),
                  AdminFileField(fileName: _bgBackName, placeholder: 'Background Image (Back) — PNG, JPG (max 2MB)', onTap: () => setState(() => _bgBackName = 'back_bg.jpg')),
                  AdminFileField(fileName: _logoName, placeholder: 'School Logo — PNG, JPG, SVG (max 1MB)', onTap: () => setState(() => _logoName = 'logo.png')),
                  AdminFileField(fileName: _signatureName, placeholder: 'Signature Image — PNG, JPG (max 1MB)', onTap: () => setState(() => _signatureName = 'signature.png')),
                  const SizedBox(height: 6),
                  Row(
                    children: [
                      WebButton(
          ...
          AdminSectionCard(
            ...
            trailing: SizedBox(
              ...
              child: TextField(
                ...
                onChanged: (v) => ref.read(idCardTemplateListProvider.notifier).setSearch(v),
              ),
            ),
            child: Column(
              children: [
                ...
                AdminPaginationBar(
                  ...
                  onPageChange: (p) => ref.read(idCardTemplateListProvider.notifier).setPage(p),
                  onPageSizeChange: (s) => ref.read(idCardTemplateListProvider.notifier).setPageSize(s),
  ...
  List<List<Widget>> _buildRows(dynamic state, List<RoleEntity> roles) {
    final notifier = ref.read(idCardTemplateListProvider.notifier);
    final visible = notifier.filtered((e, q) => e.title.toLowerCase().contains(q) || e.pageLayoutStyle.contains(q));
    ...
    return List.generate(visible.length, (index) {
      ...
      final roleBadges = e.applicableRoleIds.isEmpty
          ...
          : e.applicableRoleIds
              .map((id) => _roleBadge(roles.firstWhere((r) => r.id == id, orElse: () => RoleEntity(id: id, name: 'Role $id')).name))
              .toList();
      return [
  ...
  Widget _roleBadge(String label, {bool muted = false}) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: muted ? const Color(0xFFE5E7EB) : const Color(0xFFC7D2FE)),
        borderRadius: BorderRadius.circular(999),
      ),
      child: Text(label, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: muted ? const Color(0xFF6B7280) : const Color(0xFF1D4ED8))),
    );
  }
}
